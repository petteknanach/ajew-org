#!/usr/bin/env python3
"""Build 30 exact bilingual Torah 15 phrase studies without changing sourced words."""
from __future__ import annotations
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; BASE=ROOT/'public/reader/super/likutay-moharan/1/15'
...
def main():
    study=json.loads((BASE/'torah-study.json').read_text(encoding='utf-8')); pettek=json.loads((ROOT/'public/reader/pettek-nanach-commentary/torah-15.json').read_text(encoding='utf-8-sig').replace('\x00',''))
    ...
    (BASE/'phrase-study.json').write_text(json.dumps({'title':'Torah 15 phrase-by-phrase study guide','status':'Editorial navigation aid — sourced Hebrew and English remain textually unchanged','phrases':phrases},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('Prepared 30 exact bilingual Torah 15 phrase-study entries across the canonical witness.')
