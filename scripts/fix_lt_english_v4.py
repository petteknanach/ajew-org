#!/usr/bin/env python3
"""
Fix LT English translations - positional matching.
Since JSON and HTML have different Hebrew (different editions),
match by segment position within each prayer.
"""
    ...
    for d in HTML_DIRS:
        ...
        for f in sorted(os.listdir(d)):
            ...
            for h_idx, h_match in enumerate(headings):
                ...
                if nm:
                    eng = re.match(r'([a-zA-Z\s-]+)', nm.group(1))
                    if eng: pnum = words_to_num(eng.group(1).strip())
...
def main():
    ...
    # Process part 1
    print('\nProcessing Part 1 (positional matching)...')
    matched = 0
    ...
    for prayer_num in range(1, 153):
        ...
        # Get content segments (non-date)
        content_segs = [(i, seg) for i, seg in enumerate(data['segments'])
                       if not is_date_or_short(seg['he'])]
        if not content_segs: continue
        
        # Get HTML English paragraphs
        html_en = html_data.get(prayer_num, [])
        
        # If counts match, assign by position
        if len(html_en) == len(content_segs):
            for (idx, seg), en in zip(content_segs, html_en):
                if not seg.get('en', '').strip():
                    data['segments'][idx]['en'] = en
                    matched += 1
        elif html_en:
            # Partial match - assign what we can
            for i, (idx, seg) in enumerate(content_segs):
                if i < len(html_en) and not seg.get('en', '').strip():
