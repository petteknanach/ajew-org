#!/usr/bin/env python3
"""Build Arrogance 7-41 through Theft 1-5: 80 stills and 40 genuine Grok videos.
...
TARGETS=Path('/root/sefer_hamidos_next40_arrogance7_theft5.json')
WORKERS=[Path('/root/sh_media3_worker_1_14.json'),Path('/root/sh_media3_worker_15_27.json'),Path('/root/sh_media3_worker_28_40.json')]
W,H=1280,720
...
def main():
    ...
    for gi,target in enumerate(targets,1):
        ...
        if is_local:
            ...
            base_dir=out/'grok-bases-arrogance7-theft5'; base_path=base_dir/f'sh-{slug}-{seg:03d}-grok-base.png'
        ...
        raw_dir=out/'raw-grok-videos-arrogance7-theft5'; raw=raw_dir/f'sh-{slug}-{seg:03d}-raw-grok.mp4'; download(row['video_url'],raw,100000)
        overlay=Path('/tmp/sefer-hamidos-arrogance7-theft5-overlays')/f'sh-{slug}-{seg:03d}.png'
        ...
        for lang,label,text in [('he','hebrew',target['he']),('en','english',target['en'])]:
            ...
            if base_path:item['base_image']=f'/images/{collection}/grok-bases-arrogance7-theft5/{base_path.name}'
            images.append(item)
