#!/usr/bin/env python3
"""Repair Torah 7's coarse Likutay Tefilos records from its authoritative bilingual HTML."""
...
HTML = ROOT / 'public/teachings/likutay-tefilos/likutay_tefilos_07_prayer7.html'
OUT = ROOT / 'public/reader/likutay-tefilos/part-1/prayer-7.json'
...
def main() -> None:
    ...
    if len(segments) != 4:
        raise RuntimeError(f'Expected 4 bilingual prayer blocks, found {len(segments)}')
    data = json.loads(OUT.read_text(encoding='utf-8'))
    data['segments'] = segments
    data['hasEnglish'] = True
    data['superReaderRepairSource'] = str(HTML.relative_to(ROOT))
    OUT.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('Repaired Torah 7 Likutay Tefilos as 4 authoritative bilingual prayer blocks.')
