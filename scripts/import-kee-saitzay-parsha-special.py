...
SKIP_HE={'כי תצא','ליקוטים מליקוטי הלכות ומספרי רבנו','טקסט עברי מתוקן ומשוחזר'}
SKIP_EN={'Ki Seitzei','Kee Saitzay','Selected Teachings from Likutay Halachos and Rabbeinu’s Works','Scholastic Formal-Equivalence Translation'}
EN_FILLS={
  342: 'When Hashem your God gives you rest from all your enemies around you, the complete fall of the kelipah of Amalek will be only when Israel comes to the holy rest and inheritance, as it is written: “When Hashem your God gives you rest… you shall erase the memory of Amalek.” As long as the holy rest is not complete, Amalek rises again each time; yet he is subdued again and again through the power of Torah and the tzaddikim, especially the tzaddikim who dwell in the dust and have already entered complete rest.'
}
...
def strip_public_notes(text):
    ...
    text=text.replace('Parashas Ki Seitzei','Parashas Kee Saitzay').replace('Ki Seitzei','Kee Saitzay')
...
he=parse(DL/'Ki_Seitzei_AJew_corrected_Hebrew.docx','he')
en_raw=parse(DL/'Ki_Seitzei_AJew_formal_equivalence_English.docx','en')
# English source has one carry-over [1] from Shoftim; [2] aligns to Hebrew [1].
...
for h in he['entries']:
    ...
    entries.append({'number':h['number'],'parsha':'ki-teitzei','heHeading':h['heading'],'enHeading':e['heading'],'he':h['text'],'en':e['text'],'type':'teaching'})
data={
  'id':'kee-saitzay-ajew-study',
  'title':'Kee Saitzay AJew Study Packet',
  'hebrewTitle':'כי תצא',
  ...
  'sourceFiles': {
    'hebrew':'/mnt/c/Users/Pettek/Downloads/Ki_Seitzei_AJew_corrected_Hebrew.docx',
    'english':'/mnt/c/Users/Pettek/Downloads/Ki_Seitzei_AJew_formal_equivalence_English.docx',
    'pdf':'/pdfs/parsha/%D7%93%D7%91%D7%A8%D7%99%D7%9D/%D7%9B%D7%99%20%D7%AA%D7%A6%D7%90.pdf'
  },
  'entries': entries
}
out=ROOT/'public/data/parsha-special/kee-saitzay.json'
...
reader={'id':'parsha-packet-kee-saitzay','title':'AJew Study Packet — Parashas Kee Saitzay','hebrewTitle':'כי תצא — ליקוטים','source':data['subtitle'],'pdf':data['sourceFiles']['pdf'],'segments':segs}
rp=ROOT/'public/reader/parsha-packets/kee-saitzay.json'
rp.parent.mkdir(parents=True,exist_ok=True)
rp.write_text(json.dumps(reader,ensure_ascii=False,indent=2)+'\n')
for p in [out,rp]:
    s=p.read_text()
    bad=[x for x in ['Translator','מתרגם','Editorial note','הערת עריכה','AI agents','גרסאות','error_outline','Translation error','Parashas Ki Seitzei','Ki Seitzei'] if x in s]
    if bad:
