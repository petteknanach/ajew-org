#!/usr/bin/env python3
"""Prepare Torah 17 Pe'er manifest; convert only with explicit --convert."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/17/peer-halikutim'; START,END,PASSAGES=33,112,100
...
def page_rows():
 ...
 for off,page in enumerate(range(START,END+1)):
  ...
  rows.append({'sourcePage':page,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/17/peer-halikutim/page-{page}.webp','relatedSections':related,'relatedPassages':related,'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 return rows
def convert():
 import fitz
 from PIL import Image
 source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-17.pdf',garbage=4,deflate=True)
...
def main():
 ...
 expected={f'page-{n}.webp' for n in range(START,END+1)}; present={p.name for p in OUT.glob('page-*.webp')}; complete=present==expected and (OUT/'peer-halikutim-torah-17.pdf').is_file()
 if present and present!=expected: raise RuntimeError('Partial Torah 17 Pe’er image set exists')
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 17','hebrewTitle':'פאר הליקוטים — תורה יז','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':66038,'sourceUrl':'https://hebrewbooks.org/66038','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=66038','sourceSha256':digest,'pdf':'/reader/super/likutay-moharan/1/17/peer-halikutim/peer-halikutim-torah-17.pdf','textStatus':'facsimile-only','facsimileStatus':'ready' if complete else 'pending-separately-supervised-conversion','textNotice':'The scan is authoritative. Torah 17 is PDF pages 33–112 inclusive (80 pages); page 113 begins Torah 18 and is excluded. Page relationships are navigational.','sectionDefinitions':SECTIONS,'pages':page_rows()}
 ...
 print(f'Prepared Torah 17 Pe’er manifest: 80 pages (33–112), HebrewBooks 66038; {status}.')
