#!/usr/bin/env python3
"""Build 30 exact bilingual Torah 33 phrase studies across all nine sections."""
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; BASE=ROOT/'public/reader/super/likutay-moharan/1/33'; SPAN=re.compile(r'<span data-inline-phrase="[^"]+">(.*?)</span>')
...
def main():
 sp=BASE/'torah-study.json'; study=json.loads(sp.read_text()); p=json.loads((ROOT/'public/reader/pettek-nanach-commentary/torah-33.json').read_text(encoding='utf-8-sig')); segs=study['segments']
 if len(segs)!=53 or {int(z['sourceSection']) for z in segs}!=set(range(1,10)): raise RuntimeError('segments/sections')
 for z in segs:
