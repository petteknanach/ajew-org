#!/usr/bin/env python3
"""Prepare Torah 21 Pe'er manifest; convert only with explicit --convert."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/21/peer-halikutim'; START,END,PASSAGES=301,412,102
...
def page_rows():
 ...
 for off,page in enumerate(range(START,END+1)):
  ...
  rows.append({'sourcePage':page,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/21/peer-halikutim/page-{page}.webp','relatedSections':related,'relatedPassages':related,'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 return rows
def convert():
 import fitz
 from PIL import Image
 source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-21.pdf',garbage=4,deflate=True)
...
def main():
 ...
 expected={f'page-{n}.webp' for n in range(START,END+1)}; present={p.name for p in OUT.glob('page-*.webp')}; complete=present==expected and (OUT/'peer-halikutim-torah-21.pdf').is_file()
 if present and present!=expected: raise RuntimeError('Partial Torah 21 Pe’er image set exists')
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 21','hebrewTitle':'פאר הליקוטים — תורה כא','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':66038,'sourceUrl':'https://hebrewbooks.org/66038','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=66038','sourceSha256':digest,'pdf':'/reader/super/likutay-moharan/1/21/peer-halikutim/peer-halikutim-torah-21.pdf','textStatus':'facsimile-only','facsimileStatus':'ready' if complete else 'pending-separately-supervised-conversion','textNotice':'The scan is authoritative. Torah 21 is PDF pages 301–412 inclusive (112 pages); page 413 begins Torah 22 and is excluded. Page relationships are navigational.','sectionDefinitions':SECTIONS,'pages':page_rows()}
 ...
 print(f'Prepared Torah 21 Pe’er manifest: 112 pages (301–412), HebrewBooks 66038; {status}.')
