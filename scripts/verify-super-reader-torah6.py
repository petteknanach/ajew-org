...
def main() -> None:
    ...
    if len(segments) != 93 or [item['index'] for item in segments] != list(range(1, 94)):
        raise RuntimeError('Torah 6 must contain 93 contiguous aligned passages')
    ...
    if manifest['sourcePageRange'] != [189, 257] or len(manifest['pages']) != 69:
    ...
    print('Validated Torah 6: 93 passages, 19 classic segments, 30 phrases, repaired Parparos, 69 Pe’er pages.')
