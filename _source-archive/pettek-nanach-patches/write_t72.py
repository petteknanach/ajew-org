...
out_path = os.path.join(pnc_dir, 'torah-72.json')
...
"Thought of teshuvah → proper man → wants to travel to tzaddik → new, higher-level evil inclination arises. New yetzer = calibrated to new level. Must strengthen specifically against this new adversary.",
...
"If not strengthened against new yetzer → falls from initial desire. Evil inclination has many levels: crude for crude people; refined for elevated people. Higher level = subtler, harder to detect. Overcome crude yetzer ≠ overcome refined one.",
...
"Highest yetzer level: Tikkunei Zohar Tikkun 70, 123b — 'Even upper crown is black to Cause of Causes.' Teh 82:5 'in darkness they walk.' Evil inclination of holy tzaddikim = aspect of upper kedushah itself / holy angel / inner gevurah.",
"הַיֵּצֶר הָעֶלְיוֹן: ת\"ז תִּקּוּן ע', קכ\"ג: — 'אֲפִילוּ כֶּתֶר הָעֶלְיוֹן שָׁחוֹר לְעִלַּת הָעִלּוֹת.' תה' פ\"ב:ה 'בַּחֲשֵׁכָה יִתְהַלָּכוּ.' יֵצֶר הַצַּדִּיקִים הַקְּדוֹשִׁים = בְּחִינַת קְדֻשָּׁה עֶלְיוֹנָה עַצְמָהּ / מַלְאָךְ קָדוֹשׁ / גְּבוּרָה פְּנִימִית.",
...
"Tzaddik's trial = upper spiritual yetzer, not crude physical. Shlomo's damage = in this subtle aspect only. When dinim upon a person → must especially guard from yetzer. Dinim = power source of even physical evil inclination.",
...
"Intermediate yetzer = thin shell (kelipah daka). Ba'alei chochmah u'da'at face this level. Not crude physical, not upper holy angel. Thin veil — disguises as kedushah. Most dangerous because hardest to distinguish.",
...
"Sinai warning: Shemot 19:21 'go down, warn the people' = warning against elevated yetzer at moment of great closeness. Higher spiritual height → more intense upper yetzer. Sinai warning ≠ physical temptation; = refined spiritual desire becoming uncontrolled.",
"אַזְהָרַת סִינַי: שְׁמ' י\"ט:כא 'רֵד הָעֵד בָּעָם' = אַזְהָרָה מִיֵּצֶר עֶלְיוֹן בְּרֶגַע קִרְבָה גְּדוֹלָה. גֹּבַהּ רוּחָנִי גָּדוֹל יוֹתֵר → יֵצֶר עֶלְיוֹן עַצְמָאִי יוֹתֵר. אַזְהָרַת סִינַי ≠ תַּאֲוָה גַּסָּה; = תְּשׁוּקָה רוּחָנִית עֲדִינָה שֶׁמִּתְפָּרֶצֶת.",
...
"Most people: gross physical yetzer. But widespread: thoughts of avodah zarah (idolatry) arise during tefillah. Even though knowing it has no substance — it overpowers, causes pain, persists. Hard to remove. The yetzer of tefillah = hardest, because tefillah = moment of greatest closeness.",
"רֹב הָעוֹלָם: יֵצֶר גַּס. אֲבָל: מַחְשָׁבוֹת עֲ\"זָ בִּשְׁעַת תְּפִלָּה — רָאוּי לְרַבִּים. אַף עַל פִּי שֶׁיּוֹדֵעַ שֶׁאֵין בָּהּ מַמָּשׁ — מִתְגַּבֶּרֶת, מַכְאִיבָה, עוֹמֶדֶת. קָשֶׁה לְהָסִיר. יֵצֶר הַתְּפִלָּה = הַקָּשֶׁה מִכֹּל.",
...
data = {
    ...
    "part": 1,
    "torah": 72,
    "title": "T72 Petten Nanach Commentary - Sometimes a Thought of Teshuvah Comes (Levels of Yetzer, 7 segs)",
    "hebrewTitle": "פְּעָמִים מַחֲשָׁבַת תְּשׁוּבָה בָּאָה — רְמוֹת הַיֵּצֶר",
...
with open(lm_path, encoding='utf-8') as f:
    cdata = json.load(f)
if '72' not in cdata['1']:
    cdata['1']['72'] = {}
cdata['1']['72']['running_commentary'] = {
    "book": pnc_name,
    "slug": pnc_name,
    "status": "available",
    "url": f"/reader/{pnc_name}/torah-72.json",
    "layers": ["beginner", "intermediate", "scholarly"],
    "author": "Petten Nanach",
    "label": "Petten Nanach Running Commentary - T72 (Levels of Yetzer Hara/Teshuvah, 7 segs)"
}
with open(lm_path, 'w', encoding='utf-8') as f:
    json.dump(cdata, f, ensure_ascii=False, indent=2)
print("lm-commentaries.json updated for T72")

t72_git = 'public/reader/' + pnc_name + '/torah-72.json'
subprocess.run(['git', 'add', 'src/data/lm-commentaries.json', t72_git], cwd=repo, check=True)
r2 = subprocess.run(['git', 'commit', '-m', 'feat: T72 PNC -- Levels of yetzer hara/teshuvah/Sinai warning (7 segs)'], cwd=repo, capture_output=True, text=True)
