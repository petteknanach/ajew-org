#!/usr/bin/env python3
"""
LT English extraction - v7.
Handles English word numbers in prayer headings (e.g., "Prayer One Hundred and Eight").
"""
...
def words_to_num(text):
    """Convert English words like 'One Hundred and Eight' to 108."""
...
def get_prayer_num_from_heading(block):
    ...
    # Try digit pattern first: "Prayer 32"
    num_m = re.search(r'Prayer\s+(\d+)', text, re.I)
    if num_m:
        return int(num_m.group(1))
    
    # Try word pattern: "Prayer One Hundred and Eight" or "Prayer Thirty-Two"
    word_m = re.search(r'Prayer\s+(.+)', text, re.I)
    if word_m:
        words = word_m.group(1).strip()
        # Remove Hebrew part if present (after middot)
        words = re.split(r'[·•&middot;]', words)[0].strip()
        num = words_to_num(words)
        if num:
            return num
    
    # Try Hebrew numeral: "תְּפִלָּה לב׳"
...
def get_prayer_num_from_filename(f):
    """Extract prayer number from filename for single-prayer files."""
