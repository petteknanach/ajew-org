...
// The haskamos text from wikisource - no nikud in this text
const haskamosTexts = [
  // Haskama 1 - R' Yaakov Yitzchak of Lublin
  ...
  // Haskama 2 - R' Yisrael of Kozhnitz (Maggid of Kozhnitz)
  ...
  // Haskama 3 - R' Avraham Chaim of Zlotshov
  ...
  // Haskama 5 - R' Efraim Zalman Margaliot of Brody
...
const segments = haskamosTexts.map((text, i) => ({
  index: i + 1,
  he: text,
  en: '',
}));
...
const haskamosJson = {
  ...
  navigation: {
    ...
    next: 'lm-intro-shir',
    prevUrl: null,
    nextUrl: '/reader/likutay-moharan/1/intro'
  },
  hasNikud: false
};

const basePath = path.join(__dirname, '..', 'public', 'reader', 'likutay-moharan');
fs.writeFileSync(path.join(basePath, 'part-1', 'haskamos.json'), JSON.stringify(haskamosJson, null, 2), 'utf8');
console.log('Created: part-1/haskamos.json -', segments.length, 'paragraphs (approbations)');
