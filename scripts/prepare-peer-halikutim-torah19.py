#!/usr/bin/env python3
"""Prepare Torah 19 Pe'er manifest; convert only with explicit --convert."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/19/peer-halikutim'; START,END,PASSAGES=151,231,90
...
def convert():
 ...
 source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-19.pdf',garbage=4,deflate=True)
...
def main():
 ...
 expected={f'page-{n}.webp' for n in range(START,END+1)}; present={p.name for p in OUT.glob('page-*.webp')}; complete=present==expected and (OUT/'peer-halikutim-torah-19.pdf').is_file()
 if present and present!=expected: raise RuntimeError('Partial Torah 19 Pe’er image set exists')
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 19','hebrewTitle':'פאר הליקוטים — תורה יט','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':66038,'sourceUrl':'https://hebrewbooks.org/66038','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=66038','sourceSha256':digest,'pdf':'/reader/super/likutay-moharan/1/19/peer-halikutim/peer-halikutim-torah-19.pdf','textStatus':'facsimile-only','facsimileStatus':'ready' if complete else 'pending-separately-supervised-conversion','textNotice':'The scan is authoritative. Torah 19 is PDF pages 151–231 inclusive (81 pages); page 232 begins Torah 20 and is excluded. Page relationships are navigational.','sectionDefinitions':SECTIONS,'pages':page_rows()}
 ...
 print(f'Prepared Torah 19 Pe’er manifest: 81 pages (151–231), HebrewBooks 66038; {status}.')
