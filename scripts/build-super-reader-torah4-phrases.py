#!/usr/bin/env python3
"""Build Torah 4's phrase navigation from aligned text and synchronized commentary."""
...
SELECTED = [1, 2, 3, 5, 7, 9, 12, 15, 18, 20, 23, 25, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60, 63, 66, 69, 72, 76, 81]
...
def main() -> None:
    ...
    pettek = json.loads((ROOT / 'public/reader/pettek-nanach-commentary/torah-4.json').read_text(encoding='utf-8'))
    ...
    for position, index in enumerate(SELECTED, start=1):
        ...
        explanation = clean_commentary(str((layers.get('beginner') or {}).get('en') or (layers.get('intermediate') or {}).get('en') or ''))
        ...
        phrases.append({
            ...
            'info': explanation or f'This passage develops the movement of Torah 4 through section {classic}.',
            'source': segment['sourceRef'], 'classicSegment': classic, 'sourceRef': segment['sourceRef'],
        })
    payload = {
        'title': 'Torah 4 phrase-by-phrase study guide',
    ...
    print(f'Prepared {len(phrases)} Torah 4 phrase-study entries.')
