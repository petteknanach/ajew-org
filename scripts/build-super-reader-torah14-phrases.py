#!/usr/bin/env python3
"""Build 30 exact bilingual Torah 14 phrase studies without changing sourced words."""
from __future__ import annotations
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; BASE=ROOT/'public/reader/super/likutay-moharan/1/14'
...
def main():
    study=json.loads((BASE/'torah-study.json').read_text(encoding='utf-8')); pettek=json.loads((ROOT/'public/reader/pettek-nanach-commentary/torah-14.json').read_text(encoding='utf-8-sig').replace('\x00',''))
    ...
    bilingual=[int(x['index']) for x in study['segments'] if x.get('en') and not x.get('hebrewOnly')]
    selected=[bilingual[(i*len(bilingual))//30] for i in range(30)]
    if len(set(selected))!=30: raise RuntimeError('Phrase selection did not produce 30 unique bilingual passages')
    for index in selected:
        segment=by_index[index]; classic=int(segment['classicSegment']); layers=commentary[classic]['layers']
    ...
    (BASE/'phrase-study.json').write_text(json.dumps({'title':'Torah 14 phrase-by-phrase study guide','status':'Editorial navigation aid — sourced Hebrew and English remain textually unchanged','phrases':phrases},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('Prepared 30 exact bilingual Torah 14 phrase-study entries across the canonical witness.')
