#!/usr/bin/env python3
"""Build 30 exact bilingual Torah 23 phrase studies across 104 production passages."""
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'public/reader/super/likutay-moharan/1/23';SPAN=re.compile(r'<span data-inline-phrase="[^"]+">(.*?)</span>')
...
def main():
 study=json.loads((BASE/'torah-study.json').read_text());p=json.loads((ROOT/'public/reader/pettek-nanach-commentary/torah-23.json').read_text(encoding='utf-8-sig'));segs=study['segments']
 if len(segs)!=104:raise RuntimeError('count')
 for s in segs:
  for k in ('he','he_nikud','en'):s[k]=unspan(s.get(k,''))
 chosen=[]
 for target in [1+round(i*103/29) for i in range(30)]:
 ...
 if len(phrases)!=30 or chosen[0]!=1 or chosen[-1]!=104:raise RuntimeError(str(chosen))
 (BASE/'torah-study.json').write_text(json.dumps(study,ensure_ascii=False,indent=2)+'\n');(BASE/'phrase-study.json').write_text(json.dumps({'title':'Torah 23 phrase-by-phrase study guide','status':'Editorial navigation aid — sourced Hebrew and English remain textually unchanged','selectedPassages':chosen,'phrases':phrases},ensure_ascii=False,indent=2)+'\n');print('Prepared 30 exact bilingual Torah 23 phrase entries from passage 1 through 104.')
