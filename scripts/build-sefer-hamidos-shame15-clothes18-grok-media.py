#!/usr/bin/env python3
"""Build Shame 15-36 + Clothes 1-18: 80 stills and 40 genuine Grok videos.
...
TARGETS=Path('/root/sefer_hamidos_next40_shame15_clothes18.json')
WORKERS=[Path('/root/sh_media_worker_1_14.json'),Path('/root/sh_media_worker_15_27.json'),Path('/root/sh_media_worker_28_40.json')]
W,H=1280,720
...
def download(url,path,min_size=10000):
    ...
    if path.stat().st_size<min_size: raise RuntimeError(f'short download {path} {path.stat().st_size}')
...
def cover_image(path):
    ...
    x=max(0,(im.width-W)//2); y=max(0,(im.height-H)//2)
...
def make_video_overlay(he,en,topic_title,segment,out):
    ...
    draw.text((52,top+17),f'Sefer HaMidos · {topic_title.split("/")[0].strip()} {segment} · Grok/xAI generated video',font=title,fill=(255,231,137,255))
    avail_h=panel_h-90
...
def overlay_video(raw,overlay,out):
    if out.exists() and out.stat().st_size>120000:return
    cmd=['ffmpeg','-y','-i',str(raw),'-i',str(overlay),'-t','5','-filter_complex',
         '[0:v]scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,setsar=1,format=yuv420p[v];[v][1:v]overlay=0:0:format=auto,format=yuv420p[out]',
...
def main():
    ...
    if bad: raise SystemExit(f'generation failures {bad}')
    manifests={}
    for spec in [('shame',11,'Shame / בושה'),('clothes',12,'Clothes / בגדים')]:
        ...
        manifests[slug]=old; ensure_share_page(f'sefer-hamidos-{slug}')
    for gi,target in enumerate(targets,1):
        ...
        is_local=gi<=10
        if is_local:
            src=Path(row['local_source'])
            if not src.exists(): raise SystemExit(f'missing local source {src}')
            base=cover_image(src); source_image=str(src).replace('/mnt/c/Users/Pettek/','~/')
            variant='Local Photo'; source_label='Local Na Nach/Pictures still source · exact teaching overlaid'
            base_path=None
        else:
            base_dir=out/'grok-bases-shame15-clothes18'; base_path=base_dir/f'sh-{slug}-{seg:03d}-grok-base.png'
            download(row['image_url'],base_path,20000); base=cover_image(base_path)
            source_image=f'Grok/xAI generated base: {row["image_url"]}'; variant='Grok H'; source_label='Grok/xAI generated picture · exact teaching overlaid'
        raw_dir=out/'raw-grok-videos-shame15-clothes18'; raw=raw_dir/f'sh-{slug}-{seg:03d}-raw-grok.mp4'; download(row['video_url'],raw,100000)
        overlay=Path('/tmp/sefer-hamidos-shame15-clothes18-overlays')/f'sh-{slug}-{seg:03d}.png'
        display=target.get('displayLabel') or str(seg)
        make_video_overlay(target['he'],target['en'],target['topic_title'],display,overlay)
        final_video=out/f'sh-{slug}-{seg:03d}-grok-generated-overlay.mp4'; overlay_video(raw,overlay,final_video)
        images=[]
        for lang,label,text in [('he','hebrew',target['he']),('en','english',target['en'])]:
            ...
            if base_path:item['base_image']=f'/images/{collection}/grok-bases-shame15-clothes18/{base_path.name}'
            images.append(item)
        entry={'topic':target['topic'],'topic_number':target['topic'],'topic_title':target['topic_title'],'segment':seg,'displayLabel':display,
               ...
               'grok_image_source_url':row['image_url'],'grok_video_source_url':row['video_url']}
    ...
    for slug,m in manifests.items():
        for e in m['entries']:
            if (slug=='shame' and 15<=int(e['segment'])<=36) or (slug=='clothes' and 1<=int(e['segment'])<=18): new.append(e)
    uniq_vid={im['video_path'] for e in new for im in e['images']}
    local=[e for e in new if e.get('local_pictures_quarter')]
    saba=[e for e in local if '/Saba/' in e.get('source_image','')]
    ...
    if (len(new),sum(len(e['images']) for e in new),len(uniq_vid),len(local),len(saba),len(forbidden),len(missing))!=(40,80,40,10,8,0,0): raise SystemExit('strict audit failed')
