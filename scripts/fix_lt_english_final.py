#!/usr/bin/env python3
"""
Fix LT English translations - final version.
Matches JSON prayer N to HTML prayer N+1 (offset by 1).
Uses filename-based numbering for individual prayer files.
"""
...
def get_prayer_num_from_filename(f):
    """Extract prayer number from filename like 'likutay_tefilos_01_prayer1.html'"""
    # Try patterns: _01_prayer1, _1_prayer1, _01_, _1_
    ...
    for d in HTML_DIRS:
        ...
        for f in sorted(os.listdir(d)):
            ...
            for h_idx, h_match in enumerate(headings):
                ...
                # Try to extract number from heading
                pnum = None
...
def is_date_or_short(he):
    ...
    if len(t) <= 30 and bool(re.match(r'^[\u0590-\u05FF\s\d\u05F3]+$', t)): return True
    return False
...
def main():
    ...
    print(f'HTML prayer numbers: {sorted(html_data.keys())}')
    ...
    for prayer_num in range(1, 153):
        ...
        # Try direct match (HTML prayer N)
        html_paras = html_data.get(prayer_num, [])
        
        # Try shifted match (HTML prayer N+1) - JSON is offset by 1
        if not html_paras:
    ...
    for prayer_num in range(1, 153):
        ...
        for seg in data['segments']:
            ...
            if not seg.get('en', '').strip(): empty += 1
    ...
    for prayer_num in range(1, 60):
        ...
        data = json.load(open(json_path, 'r'))
        html_paras = html_data.get(prayer_num, [])
        if not html_paras: html_paras = html_data.get(prayer_num + 1, [])
        if not html_paras: continue
    ...
    for prayer_num in range(1, 60):
        ...
        for seg in data['segments']:
            if is_date_or_short(seg['he']): continue
            if not seg.get('en', '').strip(): empty2 += 1
