#!/usr/bin/env python3
"""Prepare authoritative Pe'er HaLikutim facsimiles for Torah 8."""
...
OUT = ROOT / 'public/reader/super/likutay-moharan/1/8/peer-halikutim'
START, END = 55, 108
PASSAGES = 97
SECTIONS = [
...
def main() -> None:
    ...
    clipped.save(OUT / 'peer-halikutim-torah-8.pdf', garbage=4, deflate=True)
    ...
    for offset, source_page in enumerate(range(START, END + 1)):
        ...
        pages.append({
            ...
            'image': f'/reader/super/likutay-moharan/1/8/peer-halikutim/page-{source_page}.webp',
    ...
    manifest = {
        'schemaVersion': 2, 'title': "Pe’er HaLikutim — Torah 8", 'hebrewTitle': 'פאר הליקוטים — תורה ז',
        ...
        'pdf': '/reader/super/likutay-moharan/1/8/peer-halikutim/peer-halikutim-torah-8.pdf',
    ...
    print(f'Prepared {len(pages)} Torah 8 Pe’er pages ({START}-{END}).')
