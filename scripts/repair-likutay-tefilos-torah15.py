#!/usr/bin/env python3
"""Rebuild Torah 15 Likutay Tefilos from ten authoritative bilingual HTML blocks."""
from __future__ import annotations
import json
from pathlib import Path
from bs4 import BeautifulSoup
ROOT=Path(__file__).resolve().parents[1]
HTML=ROOT/'public/teachings/likutay-tefilos/likutay_tefilos_15_prayer15 (1).html'
OUT=ROOT/'public/reader/likutay-tefilos/part-1/prayer-15.json'
def main():
    ...
    if len(segments)!=10: raise RuntimeError(f'Expected 10 authoritative bilingual blocks, got {len(segments)}')
    data=json.loads(OUT.read_text(encoding='utf-8-sig').replace('\x00',''))
    data.update({'id':'lt-1-15','book':'likutay-tefilos','part':1,'torah':15,'displayNumber':15,'segments':segments,'aligned_segments':segments,'totalParagraphs':10,'totalSegments':10,'hasEnglish':True,'superReaderRepairSource':str(HTML.relative_to(ROOT))})
    OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('Rebuilt Torah 15 prayer as ten authoritative bilingual blocks (lt-1-15).')
