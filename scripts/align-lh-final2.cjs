#!/usr/bin/env node
/**
 * FINAL LH English alignment - production version.
 * 
 * 1. Parse HTML titles to extract base halacha name
 * 2. Group consecutive HTML files with same base name, concatenate paragraphs
 * 3. Group consecutive Hebrew files with same base name
 * 4. Match HTML groups to Hebrew groups by ORDER of first appearance within each part
 * 5. Distribute English paragraphs proportionally across Hebrew segments
 */
const fs = require('fs');
...
function parseTitle(title) {
  ...
  // Remove all volume/section prefixes
  let prevT = '';
  while (t !== prevT) {
    prevT = t;
...
// Group consecutive entries with same name
...
console.log('Building Hebrew entries...');
...
for (let part = 1; part <= 8; part++) {
  ...
  // Group HTML entries by consecutive same name (normalized)
  const htmlGroups = groupByName(htmlEntries, e => e.name.toLowerCase(), e => e.paragraphs);
  
  // Group Hebrew entries by consecutive same base name (no paragraphs to accumulate)
  ...
  if (htmlGroups.length !== hebGroups.length) {
    console.log(`  WARNING: group count mismatch`);
...
console.log(`Unmatched: ${unmatched} Hebrew files`);
