#!/usr/bin/env python3
"""Prepare authoritative Pe'er HaLikutim facsimiles for Torah 10."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/10/peer-halikutim'
START,END,PASSAGES=151,197,70
SECTIONS=[
...
def main():
 OUT.mkdir(parents=True,exist_ok=True); source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-10.pdf',garbage=4,deflate=True)
 ...
 for off,n in enumerate(range(START,END+1)):
  ...
  pages.append({'sourcePage':n,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/10/peer-halikutim/page-{n}.webp','relatedSections':related,'relatedPassages':related,'pageBox':[round(page.rect.width,1),round(page.rect.height,1)],'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 sha=hashlib.sha256(SOURCE.read_bytes()).hexdigest()
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 10','hebrewTitle':'פאר הליקוטים — תורה י','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':54912,'sourceUrl':'https://hebrewbooks.org/54912','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=54912','sourceSha256':sha,'pdf':'/reader/super/likutay-moharan/1/10/peer-halikutim/peer-halikutim-torah-10.pdf','textStatus':'facsimile-only','textNotice':'The scan is authoritative. Page relationships are navigational and should be verified against the facsimile.','sectionDefinitions':SECTIONS,'pages':pages}
 (OUT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); source.close(); clipped.close(); print('Prepared 47 Torah 10 Pe’er pages (151–197).')
