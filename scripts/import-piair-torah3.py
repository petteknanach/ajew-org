#!/usr/bin/env python3
"""Prepare a section-aware Pe'er HaLikutim Torah 3 dataset for the Super Reader.
...
SOURCE = Path(os.environ.get('PEER_SOURCE', '/mnt/c/Users/Pettek/Downloads/Piair halikutim - likutay moharan 1 - 1-6 - Hebrewbooks_org_54911.pdf'))
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / 'public' / 'reader' / 'super' / 'likutay-moharan' / '1' / '3' / 'peer-halikutim'
FIRST_PAGE = 88
LAST_PAGE = 102

PAGE_SECTIONS = {
    88: [1, 2, 3, 4], 89: [4], 90: [4], 91: [4], 92: [4], 93: [4],
    94: [4], 95: [4, 5], 96: [5], 97: [6], 98: [6, 7, 8],
    99: [8], 100: [8, 9], 101: [9, 10, 11], 102: [11, 12],
}

# Fine-grained synchronized reading passages used by the Super Reader. These
# remain distinct from Pe’er's printed internal section numbers above.
PAGE_PASSAGES = {
...
def main() -> None:
    ...
    excerpt_path = OUT / 'peer-halikutim-torah-3.pdf'
    ...
    for source_page in range(FIRST_PAGE, LAST_PAGE + 1):
        ...
        pages.append({
            ...
            'printedFolio': None,
            'image': f'/reader/super/likutay-moharan/1/3/peer-halikutim/{filename}',
            'relatedSections': PAGE_SECTIONS[source_page],
            'relatedPassages': PAGE_PASSAGES[source_page],
    ...
    manifest = {
        'schemaVersion': 2,
        'title': "Pe’er HaLikutim — Torah 3",
        'hebrewTitle': 'פאר הליקוטים — תורה ג',
        'sourceFile': SOURCE.name,
        'sourcePageRange': [FIRST_PAGE, LAST_PAGE],
        'pdf': '/reader/super/likutay-moharan/1/3/peer-halikutim/peer-halikutim-torah-3.pdf',
