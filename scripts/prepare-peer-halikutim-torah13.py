#!/usr/bin/env python3
"""Prepare Torah 13 Pe'er manifest; convert facsimiles only with explicit --convert."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/13/peer-halikutim'; START,END,PASSAGES=282,328,81
SECTIONS=[
...
def page_rows():
 ...
 for off,page in enumerate(range(START,END+1)):
  ...
  rows.append({'sourcePage':page,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/13/peer-halikutim/page-{page}.webp','relatedSections':related,'relatedPassages':related,'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 return rows
def convert():
 import fitz
 from PIL import Image
 source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-13.pdf',garbage=4,deflate=True)
...
def main():
 ...
 expected={f'page-{n}.webp' for n in range(START,END+1)}; present={p.name for p in OUT.glob('page-*.webp')}; complete=expected==present and (OUT/'peer-halikutim-torah-13.pdf').is_file()
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 13','hebrewTitle':'פאר הליקוטים — תורה יג','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':54912,'sourceUrl':'https://hebrewbooks.org/54912','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=54912','sourceSha256':sha256(SOURCE),'pdf':'/reader/super/likutay-moharan/1/13/peer-halikutim/peer-halikutim-torah-13.pdf','textStatus':'facsimile-only','facsimileStatus':'ready' if complete else 'pending-separately-supervised-conversion','textNotice':'The scan is authoritative. Page relationships are navigational and should be verified against the facsimile.','sectionDefinitions':SECTIONS,'pages':page_rows()}
 ...
 print(f'Prepared Torah 13 Pe’er manifest: 47 pages (282–328); {status}.')
