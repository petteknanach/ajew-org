#!/usr/bin/env python3
"""Build Torah 5's phrase navigation from aligned text and synchronized commentary."""
...
SELECTED = [1, 2, 3, 5, 7, 9, 12, 14, 17, 19, 22, 24, 27, 29, 32, 34, 37, 39, 42, 44, 47, 49, 52, 54, 57, 59, 62, 65, 68, 71]
...
def main() -> None:
    ...
    pettek = json.loads((ROOT / 'public/reader/pettek-nanach-commentary/torah-5.json').read_text(encoding='utf-8'))
    ...
    for position, index in enumerate(SELECTED, start=1):
        ...
        explanation = f'“{direct_line}” — {commentary_text or f"This line develops section {classic} of Torah 5."}'
        phrases.append({
    ...
    payload = {
        'title': 'Torah 5 phrase-by-phrase study guide',
    ...
    print(f'Prepared {len(phrases)} Torah 5 phrase-study entries.')
