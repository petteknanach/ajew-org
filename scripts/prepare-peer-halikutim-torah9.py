#!/usr/bin/env python3
"""Prepare authoritative Pe'er HaLikutim facsimiles for Torah 9."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/9/peer-halikutim'
START,END,PASSAGES=109,150,63
SECTIONS=[
...
def main():
 OUT.mkdir(parents=True,exist_ok=True); source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-9.pdf',garbage=4,deflate=True)
 pages=[]; count=END-START+1
 for off,n in enumerate(range(START,END+1)):
  ...
  lo=1+(off*PASSAGES)//count; hi=max(lo,((off+1)*PASSAGES)//count)
  pages.append({'sourcePage':n,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/9/peer-halikutim/page-{n}.webp','relatedSections':list(range(lo,min(PASSAGES,hi)+1)),'relatedPassages':list(range(lo,min(PASSAGES,hi)+1)),'pageBox':[round(page.rect.width,1),round(page.rect.height,1)],'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 9','hebrewTitle':'פאר הליקוטים — תורה ט','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':54912,'sourceUrl':'https://hebrewbooks.org/54912','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=54912','sourceSha256':'fa1ed1cfe0e39e0046805f93e227c0a84e90823fb7a472b103f73edf1862acfc','pdf':'/reader/super/likutay-moharan/1/9/peer-halikutim/peer-halikutim-torah-9.pdf','textStatus':'facsimile-only','textNotice':'The scan is authoritative. Page relationships are navigational and should be verified against the facsimile.','sectionDefinitions':SECTIONS,'pages':pages}
 (OUT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); source.close(); clipped.close(); print(f'Prepared {len(pages)} Torah 9 Pe’er pages ({START}-{END}).')
