#!/usr/bin/env python3
"""
LT English extraction - v13.
Smart matching: Hebrew prefix first, then proportional positional assignment.
When JSON has more segments than HTML paragraphs, multiple JSON segments
get the same English paragraph (the one they're closest to positionally).
"""
...
def smart_match(content_indices, segments, para_pairs, html_he_list):
    """
    Smart matching strategy:
    1. Hebrew prefix match (1:1, best score wins)
    2. For unmatched JSON segments, assign by proportional position
    """
    n_json = len(content_indices)
    n_html = len(para_pairs)
    ...
    # Pass 1: Hebrew prefix matching (greedy, best score)
    assignments = {}  # json_idx -> html_idx
    used_html = set()
    
    for j_idx in content_indices:
        seg_he = normalize_compare(segments[j_idx]['he'])
        ...
        for h_idx, html_he in enumerate(html_he_list):
            ...
            prefix = min(50, len(seg_he), len(html_he))
            if seg_he[:prefix] == html_he[:prefix] and prefix > best_score:
                best_score = prefix
                best_h = h_idx
        
        if best_h >= 0:
            assignments[j_idx] = best_h
            used_html.add(best_h)
    
    # Pass 2: Proportional positional matching for unmatched
    unmatched_json = [j_idx for j_idx in content_indices if j_idx not in assignments]
    unmatched_html = [h_idx for h_idx in range(n_html) if h_idx not in used_html]
    
    if not unmatched_json:
        pass  # All matched!
    elif not unmatched_html:
        # No HTML left - assign last HTML paragraph to all remaining
        last_html = n_html - 1
        for j_idx in unmatched_json:
            assignments[j_idx] = last_html
    else:
        # Proportional assignment: map JSON position to HTML position
        # For each unmatched JSON segment, find the closest unmatched HTML by proportional position
        for j_pos, j_idx in enumerate(unmatched_json):
            # Map position in unmatched_json to position in unmatched_html
            if len(unmatched_json) <= len(unmatched_html):
                h_pos = int(j_pos * len(unmatched_html) / len(unmatched_json))
                h_pos = min(h_pos, len(unmatched_html) - 1)
            else:
                # More JSON segments than HTML - multiple JSON per HTML
                h_pos = int(j_pos * len(unmatched_html) / len(unmatched_json))
                h_pos = min(h_pos, len(unmatched_html) - 1)
            assignments[j_idx] = unmatched_html[h_pos]
...
def process_part(part_dir, html_data, max_num):
    ...
    for prayer_num in range(1, max_num + 1):
        ...
        assignments = smart_match(content_indices, data['segments'], para_pairs, html_he_list)
        
        for j_idx, h_idx in assignments.items():
