#!/usr/bin/env python3
"""
Fix LT English translations - fill missing from HTML source.
JSON prayer N matches HTML prayer N+1 (offset by 1 for prayers 30+).
"""
    ...
    for d in HTML_DIRS:
        ...
        for f in sorted(os.listdir(d)):
            ...
            for h_idx, h_match in enumerate(headings):
                ...
                if nm:
                    # Get only English words (number part)
                    eng = re.match(r'([a-zA-Z\s-]+)', nm.group(1))
                    if eng:
                        pnum = words_to_num(eng.group(1).strip())
...
def process_part(part_dir, html_data, max_num):
    ...
    for prayer_num in range(1, max_num + 1):
        ...
        # Find segments that need English (don't clear existing)
