...
const files = [
  ...
  '/mnt/c/Users/Pettek/LikutayHalachos - project by claude/English/Likutay Halachos/Likutay Halachos - Yoreh Daya - 1/190 LH_YD_meat milk 5 - taaruvos - maachalei_akum_hechsher_keilim (1).html',
];

for (const f of files) {
  if (!fs.existsSync(f)) { console.log('NOT FOUND:', path.basename(f)); continue; }
  ...
  if (halachaNums.length > 1) console.log(`  Halacha numbers: ${halachaNums.join(', ')}`);
}
