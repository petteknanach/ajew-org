#!/usr/bin/env python3
"""
LT English extraction - v6.
Always split HTML at prayer-heading boundaries.
For single-heading files, use filename to determine prayer number.
"""
...
def get_prayer_num_from_heading(block):
    ...
    # "Prayer 32" or "Prayer Thirty-Two"
    num_m = re.search(r'Prayer\s+(\d+)', text, re.I)
    if num_m:
        return int(num_m.group(1))
    # Try Hebrew numeral patterns like "תְּפִלָּה לב׳"
    ...
    for d in HTML_DIRS:
        ...
        for f in sorted(os.listdir(d)):
            ...
            if len(headings) == 0:
                ...
                # Single prayer - try heading first, then filename
