#!/usr/bin/env python3
"""Build Clothes 19-20 through Arrogance 1-6: 80 stills and 40 genuine Grok videos.
...
TARGETS=Path('/root/sefer_hamidos_next40_clothes19_arrogance6.json')
WORKERS=[Path('/root/sh_media2_worker_1_14.json'),Path('/root/sh_media2_worker_15_27.json'),Path('/root/sh_media2_worker_28_40.json')]
W,H=1280,720
...
def main():
    ...
    manifests={}
    for spec in [
        ('clothes',12,'Clothes / בגדים'),
        ('trust-in-g-d',13,'TRUST (in G-d) / בטחון'),
        ('tiding',14,'Tiding / בשורה'),
        ('blessing-part-ii',15,'Blessing (Part II) / ברכה'),
        ('crying-part-ii',16,'Crying (Part II) / בכיה'),
        ('arrogance',17,'Arrogance / גאוה'),
    ]:
    ...
    for gi,target in enumerate(targets,1):
        ...
        if not row.get('image_url') or not row.get('video_url'):
            continue
        is_local=bool(row.get('local_source'))
        if is_local:
            ...
            base_dir=out/'grok-bases-clothes19-arrogance6'; base_path=base_dir/f'sh-{slug}-{seg:03d}-grok-base.png'
        ...
        raw_dir=out/'raw-grok-videos-clothes19-arrogance6'; raw=raw_dir/f'sh-{slug}-{seg:03d}-raw-grok.mp4'; download(row['video_url'],raw,100000)
        overlay=Path('/tmp/sefer-hamidos-clothes19-arrogance6-overlays')/f'sh-{slug}-{seg:03d}.png'
        ...
        for lang,label,text in [('he','hebrew',target['he']),('en','english',target['en'])]:
            ...
            if base_path:item['base_image']=f'/images/{collection}/grok-bases-clothes19-arrogance6/{base_path.name}'
            images.append(item)
