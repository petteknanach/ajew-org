#!/usr/bin/env python3
"""Prepare authoritative Pe'er HaLikutim facsimiles for Torah 4."""
...
OUT = ROOT / 'public/reader/super/likutay-moharan/1/4/peer-halikutim'
START, END = 103, 152
PASSAGES = 81
SECTIONS = [
...
def main() -> None:
    ...
    clipped.save(OUT / 'peer-halikutim-torah-4.pdf', garbage=4, deflate=True)
    ...
    for offset, source_page in enumerate(range(START, END + 1)):
        ...
        pages.append({
            ...
            'image': f'/reader/super/likutay-moharan/1/4/peer-halikutim/page-{source_page}.webp',
    ...
    manifest = {
        'schemaVersion': 2, 'title': "Pe’er HaLikutim — Torah 4", 'hebrewTitle': 'פאר הליקוטים — תורה ד',
        'sourceFile': SOURCE.name, 'sourcePageRange': [START, END],
        'pdf': '/reader/super/likutay-moharan/1/4/peer-halikutim/peer-halikutim-torah-4.pdf',
    ...
    print(f'Prepared {len(pages)} Torah 4 Pe’er pages ({START}-{END}).')
