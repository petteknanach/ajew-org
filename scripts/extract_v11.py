#!/usr/bin/env python3
"""
LT English extraction - v11.
Uses correct HTML source for each part:
- Part 1 JSON -> Lekutay Tefilos 1 directory + teachings directory
- Part 2 JSON -> Likutay Tefilos 2 directory
"""
...
# Separate HTML sources for each part
PART1_HTML_DIRS = [
...
def main():
    ...
    for prayer_num in range(1, 153):
        ...
        if prayer_num not in part1_data:
            continue
        
        para_pairs = part1_data[prayer_num]
        if not para_pairs:
            continue
    ...
    print(f'  Matched: {total_matched}/{total_content}')
    ...
    for prayer_num in range(1, 60):
        ...
        if prayer_num not in part2_data:
            continue
        
        para_pairs = part2_data[prayer_num]
        if not para_pairs:
            continue
    ...
    print(f'  Matched: {part2_matched}/{part2_content}')
    
    total_all = total_matched + part2_matched
    content_all = total_content + part2_content
    print(f'\\nTotal: {total_all}/{content_all} ({total_all/content_all*100:.1f}%)' if content_all > 0 else 'N/A')
