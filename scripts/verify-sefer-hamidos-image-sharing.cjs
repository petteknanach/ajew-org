...
const assertions = [
  ...
  [source.includes("desktopCopyPromise = navigator.clipboard.write"), 'Desktop X must copy the actual picture when clipboard image writes are supported'],
  [source.includes("Picture downloaded — upload it to ' + app"), 'Desktop X and Instagram must download the actual picture as fallback'],
