#!/usr/bin/env python3
"""Build 30 exact bilingual Torah 31 phrase studies across all eighteen Sefaria sections."""
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; BASE=ROOT/'public/reader/super/likutay-moharan/1/31'; SPAN=re.compile(r'<span data-inline-phrase="[^"]+">(.*?)</span>')
...
def main():
 study_path=BASE/'torah-study.json'; study=json.loads(study_path.read_text(encoding='utf-8')); pettek=json.loads((ROOT/'public/reader/pettek-nanach-commentary/torah-31.json').read_text(encoding='utf-8-sig')); segments=study['segments']
 if len(segments)!=109 or {int(z['sourceSection']) for z in segments if isinstance(z.get('sourceComment'),int)}!=set(range(1,19)): raise RuntimeError('study sections')
 ...
 phrases=[]; distribution={int(z['index']):(2 if int(z['sourceSection'])<=12 else 1) for z in selected}
 ...
 if len(phrases)!=30 or {z['section'] for z in phrases}!=set(range(1,19)): raise RuntimeError('phrase coverage')
 study_path.write_text(json.dumps(study,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); (BASE/'phrase-study.json').write_text(json.dumps({'title':'Torah 31 phrase-by-phrase study guide','status':'Editorial navigation aid — sourced Hebrew and English remain textually unchanged','selectedPassages':[int(z['index']) for z in selected],'representedSections':list(range(1,19)),'distribution':distribution,'phrases':phrases},ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(f'Prepared 30 exact bilingual Torah 31 phrase entries across all 18 sections; passages {[z["index"] for z in selected]}.')
