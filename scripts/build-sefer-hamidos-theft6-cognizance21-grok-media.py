#!/usr/bin/env python3
"""Build Theft 6-24 through Cognizance 1-21: 80 stills and 40 genuine Grok videos.
...
TARGETS=Path('/root/sefer_hamidos_next40_theft6_cognizance21.json')
WORKERS=[Path('/root/sh_media4_worker_1_14.json'),Path('/root/sh_media4_worker_15_27.json'),Path('/root/sh_media4_worker_28_40.json')]
W,H=1280,720
...
def main():
    ...
    manifests={}
    for spec in [
        ('theft',18,'THEFT (secretly and openly) / גנבה וגזלה'),
        ('cognizance',19,'Cognizance / דעת'),
    ]:
    ...
    for gi,target in enumerate(targets,1):
        ...
        if is_local:
            ...
            base_dir=out/'grok-bases-theft6-cognizance21'; base_path=base_dir/f'sh-{slug}-{seg:03d}-grok-base.png'
        ...
        raw_dir=out/'raw-grok-videos-theft6-cognizance21'; raw=raw_dir/f'sh-{slug}-{seg:03d}-raw-grok.mp4'; download(row['video_url'],raw,100000)
        overlay=Path('/tmp/sefer-hamidos-theft6-cognizance21-overlays')/f'sh-{slug}-{seg:03d}.png'
        ...
        for lang,label,text in [('he','hebrew',target['he']),('en','english',target['en'])]:
            ...
            item={'language':label,'variant':variant,'path':f'/images/{collection}/{fname}?v=20260727a','archive_filename':fname,
            ...
            if base_path:item['base_image']=f'/images/{collection}/grok-bases-theft6-cognizance21/{base_path.name}'
            images.append(item)
