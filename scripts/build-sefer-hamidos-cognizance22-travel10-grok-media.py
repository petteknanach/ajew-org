#!/usr/bin/env python3
"""Build Cognizance 22-51 through Travel 1-10: 80 stills and 40 genuine Grok videos.
...
TARGETS=Path('/root/sefer_hamidos_next40_cognizance22_51_travel1_10.json')
WORKERS=[Path(f'/root/sh_media5_worker_{a}_{b}.json') for a,b in [(1,5),(6,10),(11,15),(16,20),(21,25),(26,30),(31,35),(36,40)]]
W,H=1280,720
...
def main():
    ...
    for gi,target in enumerate(targets,1):
        ...
        if is_local:
            ...
            base=cover_image(src)
            local_dir=out/'local-bases-cognizance22-travel10'; local_dir.mkdir(parents=True,exist_ok=True)
            local_copy=local_dir/f'sh-{slug}-{seg:03d}-local-original{src.suffix.lower()}'
            shutil.copy2(src,local_copy)
            source_image=f'/images/{collection}/local-bases-cognizance22-travel10/{local_copy.name}'
            variant='Local Photo'; source_label='Local Na Nach/Pictures still source · exact teaching overlaid'
            base_path=None
        else:
            base_dir=out/'grok-bases-cognizance22-travel10'; base_path=base_dir/f'sh-{slug}-{seg:03d}-grok-base.png'
            download(row['image_url'],base_path,20000); base=cover_image(base_path)
            source_image=f'Grok/xAI generated base: {row["image_url"]}'; variant='Grok I'; source_label='Grok/xAI generated picture · exact teaching overlaid'
        raw_dir=out/'raw-grok-videos-cognizance22-travel10'; raw=raw_dir/f'sh-{slug}-{seg:03d}-raw-grok.mp4'; download(row['video_url'],raw,100000)
        overlay=Path('/tmp/sefer-hamidos-cognizance22-travel10-overlays')/f'sh-{slug}-{seg:03d}.png'
        ...
        for lang,label,text in [('he','hebrew',target['he']),('en','english',target['en'])]:
            ...
            item={'language':label,'variant':variant,'path':f'/images/{collection}/{fname}?v=20260728b','archive_filename':fname,
            ...
            if base_path:item['base_image']=f'/images/{collection}/grok-bases-cognizance22-travel10/{base_path.name}'
