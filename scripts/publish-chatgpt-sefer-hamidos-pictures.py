...
def classify(name: str):
    low = name.lower()
    if 'sweetening' in low and ('judgment' in low or 'judgement' in low): topic = 'sweetening'
    ...
    m = re.search(r'(?:sweetening(?:_of_judgments?)?|seclusion(?:_hisbodidus)?|thoughts?|superiority)[_-](\d{1,3})', low)
