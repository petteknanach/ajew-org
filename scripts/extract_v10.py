#!/usr/bin/env python3
"""
LT English extraction - v10.
Allows one HTML paragraph to match multiple JSON segments.
Also extracts date-bar text for date header segments.
"""
...
def extract_date_bar_texts(html_block):
    """Extract Hebrew text from date-bar divs."""
    texts = []
    for m in re.finditer(r'<div\s+class="date-bar"[^>]*>([\s\S]*?)</div>', html_block, re.I):
        text = strip_html(m.group(1))
        # Get Hebrew parts
        hebrew_parts = re.findall(r'[\u0590-\u05FF\u05C0-\u05FF\uFB00-\uFB4F\s]+', text)
...
def get_prayer_nums_from_heading(block):
    """Extract prayer number(s) from a heading block."""
    m = re.search(r'<div\s+class="prayer-heading"[^>]*>([\s\S]*?)</div>', block, re.I)
    if not m:
        return []
    text = strip_html(m.group(1))
    
    # Try ampersand pattern: "Sixty-Eight & Sixty-Nine"
    ...
    for d in HTML_DIRS:
        ...
        for f in sorted(os.listdir(d)):
            ...
            if len(headings) == 0:
                ...
                dates = extract_date_bar_texts(html)
                ...
                for num in nums:
                    if num > 0:
                        prayer_data[num] = {'pairs': pairs, 'dates': dates}
            else:
                for h_idx, h_match in enumerate(headings):
                    ...
                    dates = extract_date_bar_texts(block)
...
def main():
    ...
    for part_dir in [PART1_DIR, PART2_DIR]:
        ...
        for prayer_num in range(1, max_num + 1):
            ...
            pdata = prayer_data[prayer_num]
            para_pairs = pdata['pairs']
            date_texts = pdata['dates']
            if not para_pairs:
                continue
            ...
            # Match each JSON segment to best HTML paragraph (allowing reuse)
