#!/usr/bin/env python3
"""Prepare Torah 22 Pe'er manifest; convert only with explicit --convert."""
...
OUT=ROOT/'public/reader/super/likutay-moharan/1/22/peer-halikutim'; START,END,PASSAGES=413,516,124
...
def page_rows():
 ...
 for off,page in enumerate(range(START,END+1)):
  ...
  rows.append({'sourcePage':page,'printedFolio':None,'image':f'/reader/super/likutay-moharan/1/22/peer-halikutim/page-{page}.webp','relatedSections':related,'relatedPassages':related,'extraction':{'status':'not-rendered','fragmentCounts':{}},'fragments':[]})
 return rows
def convert():
 import fitz
 from PIL import Image
 source=fitz.open(SOURCE); clipped=fitz.open(); clipped.insert_pdf(source,from_page=START-1,to_page=END-1); clipped.save(OUT/'peer-halikutim-torah-22.pdf',garbage=4,deflate=True)
...
def main():
 ...
 expected={f'page-{n}.webp' for n in range(START,END+1)}; present={p.name for p in OUT.glob('page-*.webp')}; complete=present==expected and (OUT/'peer-halikutim-torah-22.pdf').is_file()
 if present and present!=expected: raise RuntimeError('Partial Torah 22 Pe’er image set exists')
 manifest={'schemaVersion':2,'title':'Pe’er HaLikutim — Torah 22','hebrewTitle':'פאר הליקוטים — תורה כב','sourceFile':SOURCE.name,'sourcePageRange':[START,END],'hebrewBooksId':66038,'sourceUrl':'https://hebrewbooks.org/66038','downloadUrl':'https://download.hebrewbooks.org/downloadhandler.ashx?req=66038','sourceSha256':digest,'pdf':'/reader/super/likutay-moharan/1/22/peer-halikutim/peer-halikutim-torah-22.pdf','textStatus':'facsimile-only','facsimileStatus':'ready' if complete else 'pending-separately-supervised-conversion','textNotice':'The scan is authoritative. Torah 22 is PDF pages 413–516 inclusive (104 pages); page 412 is Torah 21 and page 517 begins appendices, so both are excluded. Page relationships are navigational.','sectionDefinitions':SECTIONS,'pages':page_rows()}
 ...
 print(f'Prepared Torah 22 Pe’er manifest: 104 pages (413–516), HebrewBooks 66038; {status}.')
